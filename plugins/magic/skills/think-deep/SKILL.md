...
3. **Bind the step tracker — once, here.** Compose with the environment's native tracker when present; emulate it with a file only when it is genuinely absent (the prime directive, applied to tracking). Take the first tier below that resolves, and **tag it tersely on the Phase-1 heading** — `(task tracking: **native**)` or `(task tracking: **file protocol**)`, nothing more: no "bound", no "schema loaded", no rationale ("tools loaded…", "no hint"), no "registering the plan…" preamble.
